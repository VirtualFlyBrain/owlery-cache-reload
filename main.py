...
queries = [
    ...
    {"name": "Owlery Neurons Synaptic", "template": "http://owl.virtualflybrain.org/kbs/vfb/subclasses?object=%3Chttp://purl.obolibrary.org/obo/FBbt_00005106%3E%20and%20%3Chttp://purl.obolibrary.org/obo/RO_0002130%3E%20some%20%3Chttp://purl.obolibrary.org/obo/{id}%3E&direct=false&includeDeprecated=false&includeEquivalent=true", "id_required": True, "id_filter": _filter_class_anatomy, "allow_fallback": False},  # Was RO_0002113 (dup of Presynaptic) wrapped with includeEquivalent=false -- no live vfb.xmi query has that shape. Renamed + repointed to RO_0002130 to match the real Owlery Neurons Synaptic CompoundRefQuery in geppetto-vfb/model/vfb.xmi.
    ...
    {"name": "subClassOf cell that overlaps some X", "template": "http://owl.virtualflybrain.org/kbs/vfb/subclasses?object=%3Chttp://purl.obolibrary.org/obo/FBbt_00007002%3E%20and%20%3Chttp://purl.obolibrary.org/obo/RO_0002131%3E%20some%20%3Chttp://purl.obolibrary.org/obo/{id}%3E&direct=false&includeDeprecated=false&includeEquivalent=true", "id_required": True, "id_filter": _filter_class_anatomy},  # Was anchored on CL_0000000 (Cell Ontology) -- vfb.xmi anchors this on FBbt_00007002 (VFB cell), so the CL_0000000 warm was never read (see vfb-owlery-load audit). Renamed to match vfb.xmi's CellThatOverlapsX. NB vfb.xmi separately defines a near-duplicate CellOverlapsX (also named "subClassOf cell overlaps some X", includeEquivalent=false, with a double-escaped &amp;amp; in its XMI query string) -- left untouched pending confirmation it is actually live and well-formed.
    ...
    {"name": "Images of neurons that develops from this", "template": "http://owl.virtualflybrain.org/kbs/vfb/instances?object=%3Chttp://purl.obolibrary.org/obo/FBbt_00005106%3E%20and%20%3Chttp://purl.obolibrary.org/obo/RO_0002202%3E%20some%20%3Chttp://purl.obolibrary.org/obo/{id}%3E&direct=false&includeDeprecated=false", "id_required": True, "id_filter": _filter_class_anatomy},  # Was BFO_0000050 (part_of) -- vfb.xmi's ImagesOfNeuronsThatDevelopFromThis uses RO_0002202 (develops_from); the part_of shape never matched any live query.
